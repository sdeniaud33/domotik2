...
public:
  // ----- Constructor -----
  ButtonHandler(RS485Connector connector, byte buttonId, byte pin) : _connector(connector),
                                                                     _pin(pin),
                                                                     _buttonId(buttonId)
  {
#if DEBUGMSG
  ...
  void loop()
  {
    ...
    { // waiting for menu pin being released.
      ...
      else if ((buttonLevel == _buttonPressed) && ((unsigned long)(now - _startTime) > _pressTicks))
      {
        ...
        if (_pressFunc)
          _pressFunc(_buttonId);
        _longPressStart(_buttonId);
        if (_duringLongPressFunc)
          _duringLongPressFunc(_buttonId);
